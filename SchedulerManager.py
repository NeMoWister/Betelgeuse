...
class SchedulerManager:
    def __init__(self, db_name: str = 'c:/soft/DiscordBot/scheduler.db'):
    ...
    def get_thematic_day(self, server_id: int) -> Optional[str]:
        self.cursor.execute(
            'SELECT thematic_day_subject FROM thematic_day WHERE server_id=? AND thematic_day_date=?',
            (server_id, datetime.date.today().strftime('%Y-%m-%d')))
