...
class ImageSelector:
    def __init__(self, data: json, mychoice=None):
        ...
        self.data = data
        if mychoice is None:
            self.mychoice = choices(self.data['object_list'], weights=[item['chance'] for item in self.data['object_list']], k=1)[0]
        else:
            self.mychoice = [item for item in self.data['object_list'] if item['name'] == mychoice][0]
